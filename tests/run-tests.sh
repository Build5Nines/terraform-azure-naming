...
while IFS= read -r case_json || [[ -n "$case_json" ]]; do
	...
	for key in $expected_keys; do
			...
			if [[ $actual_status -ne 0 ]]; then
				...
				echo "  terraform version:"; $TF_CMD version || true
				echo "  test module directory listing:"; ls -la "$TEST_DIR" || true
				echo "  generated tfvars file contents:"; sed -n '1,200p' "$tfvars_file" || true
				echo "  --- terraform apply log (full) ---"
				sed -n '1,2000p' "$log_file" || true
				echo "  --- end apply log ---"
				echo "  terraform output (raw json):"; (cd "$TEST_DIR" && $TF_CMD output -json) || true
				echo "  terraform state list:"; (cd "$TEST_DIR" && $TF_CMD state list) || true
